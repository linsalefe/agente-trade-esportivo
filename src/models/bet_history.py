...
class BetHistory:
    """Gerencia histórico de apostas usando PostgreSQL"""
    ...
    def add_bet(self, bet_data: Dict) -> str:
        ...
        bet_id = f"BET_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"  # Adicionado microsegundos
        
        with get_db() as db:
            query = text("""
                INSERT INTO bets (bet_id, match, competition, market, odds, stake, 
                                 probability, ev, phase, status)
                VALUES (:bet_id, :match, :competition, :market, :odds, :stake,
                       :probability, :ev, :phase, :status)
            """)
    ...
    def update_bet_result(self, bet_id: str, result: str) -> bool:
        ...
        with get_db() as db:
            ...
            update_query = text("""
                UPDATE bets 
                SET status = :status, result = :result, profit = :profit, 
                    closed_at = NOW()
                WHERE bet_id = :bet_id
            """)
    ...
    def get_pending_bets(self) -> List[Dict]:
        ...
        with get_db() as db:
            query = text("SELECT * FROM bets WHERE status = 'pending' ORDER BY timestamp DESC")
    ...
    def get_statistics(self, phase: Optional[int] = None) -> Dict:
        ...
        with get_db() as db:
            ...
            return {
                ...
                'void': result.void or 0,
                'win_rate': round((result.won / result.total) * 100, 2) if result.total > 0 else 0,
